...
{    
    ...
    floor->set_transform(rtm::translation(0, 0, 3) * rtm::rotation_x(rtm::constants::pi/2));

    auto white = rtm::Color(1, 1, 1);
    auto black = rtm::Color(0, 0, 0);
    auto pattern = rtm::Pattern(white, black);
    
    auto matfloor = rtm::Material();
    matfloor.set_pattern(pattern);
